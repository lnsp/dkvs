...
func (master *Master) handle(m *remote.Slave) error {
	...
	for master.KeepAlive {
		...
		if cmd.KindOf(remote.CommandCluster) {
			...
			if err := master.ClusterSet.All(func(n nodes.Slave) error {
				k, err := n.LocalKeys()
				if err != nil {
					return err
				}
			...
			if err := m.Push(cmd.Param(keyList...)); err != nil {
				continue
			}
			...
			if err := master.Store(key, value, result); err != nil {
				m.Push(remote.Error(err))
				continue
			}
...
func (master *Master) Read(key string) (string, nodes.Revision, error) {
	...
	if master.ReplicationFactor > 1 {
		...
		if err := master.ClusterSet.TrialSelected(targets, func(slave nodes.Slave) error {
		...
		if err := master.ClusterSet.Trial(func(slave nodes.Slave) error {
			val, rev, err := slave.LocalRead(key)
			if err != nil {
				return err
			}
...
func (master *Master) Store(key, value string, rev nodes.Revision) error {
	if master.ReplicationFactor > 1 {
		...
		if err := master.ClusterSet.Selected(targets, func(slave nodes.Slave) error {
			return slave.LocalStore(key, value, rev)
		}); err != nil {
			return err
		}
	} else {
		if err := master.ClusterSet.All(func(slave nodes.Slave) error {
			return slave.LocalStore(key, value, rev)
		}); err != nil {
			return err
		}
...
func (master *Master) Assist(p nodes.Master) error {
	...
	if master.Primary {
		master.ClusterSet.Trial(func(peer nodes.Slave) error {
			if peer.Address() == master.Address() {
				return errSameInstance
			}
			if peer.Address() == p.Address() {
...
func (master *Master) Join(p nodes.Slave) error {
	if !master.ClusterSet.Has(p) {
		master.ClusterSet.Join(p)
		mirrors := cluster.New()
		if master.ReplicationFactor > 1 {
			for i := 0; i < master.ClusterSet.Size(); i++ {
				set := master.ClusterSet.FilterSelected(master.ClusterGroup(i), func(n nodes.Slave) bool { return n.Address() != p.Address() })
				mirrors.Union(set)
			}
		} else {
